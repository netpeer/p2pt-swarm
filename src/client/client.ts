...
export default class P2PTClient extends P2PT {
  #discovered: {[index: string]: P2PTPeer} = {
  ...
  constructor(peerId, networkVersion = 'leofcoin:peach', stars = ['wss://peach.leofcoin.org']) {
    ...
    // @ts-ignore
    this.on('data', async (peer: P2PTPeer, data) => {
  ...
  #onTrackerconnect = async (tracker: any, stats: any) => {
    ...
    let promises = Object.entries(peers).map(async ([id, peer]) => {
      ...
      if (!hasPeer && this.#discovered[id].connected) globalThis.pubsub.publish('peer:discovered', this.#discovered[id]);
    })
    await Promise.allSettled(promises)
    globalThis.pubsub.publish('star:connected', tracker)
  }

  #onPeerconnect = async (peer: P2PTPeer) => {
    if (!this.#discovered[peer.id]) this.#discovered[peer.id] = new P2PTPeer(peer, this)
    if (this.#discovered[peer.id].connected) globalThis.pubsub.publish('peer:discovered', this.#discovered[peer.id])
  ...
  #onPeerclose = async (peer: P2PTPeer) => {
  ...
  #onMessage = async (peer: P2PTPeer, data, id: PeerId, from: PeerId) => {
    if (!this.#discovered[peer.id] ||
